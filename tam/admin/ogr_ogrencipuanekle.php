...
<script type="text/javascript">
 ...
 <h2>PUAN EKLEYECEĞİN ÖĞRENCİYİ SEÇ</h2>
        ...
        try {
            ...
            if ($ogrenciler) {
                ...
                foreach ($ogrenciler as $ogrenci) {
                    ...
                    echo "<tr class='ara'>";
                    echo "<td>" . $counter .  "</td>";
                   /* echo "<td class='ara2'><img style='width: 40%; border-radius: 12px; ' src='".$ogrenci['gorsel']."'></td>";
                    Burada normalde böyle yapacaktım vazfectim hata alırıs ilk ogr_id göre gorseller tablomdan sorgu yapıp iligli
                    gorsel sutunundan veriyi çekmem gerek */

                     // İlgili öğrencinin gorsel bilgisini sorgula
                    $ogr_id = $ogrenci['ogr_id'];
                    $gorsel_sorgu = $db->prepare("SELECT gorsel FROM gorseller WHERE ogr_id = ?");
                    $gorsel_sorgu->execute([$ogr_id]);
                    $gorsel = $gorsel_sorgu->fetch(PDO::FETCH_ASSOC);

                    // Eğer görsel bulunduysa, görseli listele
                    if ($gorsel) {
                        echo "<td class='ara2'><img style='width: 30%; border-radius: 12px; ' src='../obs/l/ogrenciimg/".$gorsel['gorsel']."'></td>";
                    } else {
                        echo "<td><img style='width: 30%; border-radius: 12px; ' src='../obs/l/ogrenciimg/kayitsiz.png'></td>";
                    }
                                        echo "<td class='ara2'>".$ogrenci['ogr_ad']." <br>".$ogrenci['ogr_soyad']."</td>";
                    ...
                    echo "<td><a class='edit-button' href='ogr_puanekle.php?ogr_id=".$ogrenci['ogr_id']."'>Puan Ekle</a></td>";
                    echo "</tr>";
                }
            } else {
