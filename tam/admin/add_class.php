...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    try {
        $stmt = $db->prepare("INSERT INTO dersliksinif (dersliksinif) VALUES (:sinif_adi)");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id']) && isset($_POST['updatedValue'])) {
    ...
    try {
        $stmt = $db->prepare("UPDATE dersliksinif SET dersliksinif = :updatedValue WHERE id = :id");
...
?>
  ...
  <div class="container">
        <h2 style="text-align:center; color: white;">Yeni Sınıf Ekle</h2>
   <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
    <label for="sınıf_adı">Sınıf Adı:</label>
    <input type="text" id="sınıf_adı" name="sınıf_adı">
    <button type="submit" class="btn" name="submit">Sınıfı Ekle</button>
</form>


        <?php
        $sinif_sayisi = $db->query("SELECT COUNT(*) FROM dersliksinif")->fetchColumn();
        echo "<p style='text-align:center; color: white;'>Toplam sınıf sayısı: $sinif_sayisi</p>";
        ?>

    </div>

    <div class="container list-container">
    <h2 style="text-align:center;">Sınıflar</h2>
    <ul>
      <?php
$siniflar = $db->query("SELECT * FROM dersliksinif")->fetchAll(PDO::FETCH_ASSOC);

foreach ($siniflar as $sinif) {
    echo "<li>{$sinif['dersliksinif']} 
    ...
    deleteIcons.forEach(icon => {
        icon.addEventListener('click', function() {
            ...
            if (confirm('Bu sınıfı silmek istediğinize emin misiniz?')) {
                fetch('ds/deletesinifderslik.php', {
                    method: 'POST',
