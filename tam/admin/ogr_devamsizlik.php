...
// bu kodda amaç formdan veriyi almak ekstra artan veya azalan demesine göre if ekleme
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Formdan gönderilen verileri işle
    $ogr_id = $_POST['ogr_id'];
    $devamsizlikgim = $_POST['devamsizlikgim'];
    $kalandevamsizlik = $_POST['kalandevamsizlik'];
    $devamsizlik_durumu = $_POST['devamsizlik_durumu'];
    
    
   
    if($devamsizlikgim <= 20 && $kalandevamsizlik >= 0){

     if($devamsizlik_durumu == 1) {
    $devamsizlik_durumu = $devamsizlik_durumu + $devamsizlikgim;
    $kalandevamsizlik =  $kalandevamsizlik - 1 ; 
    
  
   
    try {
        // devamsizlik durumunu güncelle
        $query = "UPDATE devamsizlik SET devamsizlikgim = ?, kalandevamsizlik = ? WHERE ogr_id = ?";
        $statement = $db->prepare($query);
        $statement->execute([$devamsizlik_durumu, $kalandevamsizlik, $ogr_id]);
    ...
    elseif ($devamsizlik_durumu == -1) {



    $devamsizlik_durumu = $devamsizlikgim - 1;
    $kalandevamsizlik =  $kalandevamsizlik + 1 ; 
  ...
  <center>  <h2>Taksit Ödeme Durumu</h2> </center>
    <table>
        <tr><th>Sıra</th>
            ...
            <th>TC Kimlik No</th>
            <th>Devamsızlığı</th>
            <th>Kalan Devamsızlık Hakkı</th>
            <th>Devamsızlık Artır-Azalt</th>
        </tr>
       <?php
include("../connect.php");
...
try {
    ...
    if ($ogrenciler) {
        ...
        foreach ($ogrenciler as $ogrenci) {
            ...
            $devamsizlik_sorgu = $db->prepare("SELECT toplamdevamsizlik, devamsizlikgim, kalandevamsizlik FROM devamsizlik WHERE ogr_id = ?");
            $devamsizlik_sorgu->execute([$ogr_id]);
            $devamsizlik = $devamsizlik_sorgu->fetch(PDO::FETCH_ASSOC);

            // Eğer veli bulunduysa, bilgileri listele
            if ($devamsizlik) {
                echo "<td class='ara2'>".$devamsizlik['devamsizlikgim']."</td>";
                echo "<td class='ara2'>".$devamsizlik['kalandevamsizlik']."</td>";
                ...
                <form action='".$_SERVER['PHP_SELF']."' method='post'>
                 <input type='hidden' name='kalandevamsizlik' value='".$devamsizlik['kalandevamsizlik']."'>
                    <input type='hidden' name='devamsizlikgim' value='".$devamsizlik['devamsizlikgim']."'>
                  <input type='hidden' name='ogr_id' value='".$ogrenci['ogr_id']."'>
                    <button type='submit' class='edit-button' name='devamsizlik_durumu' value='1'>Arttır</button>
                    <button type='submit' class='edit-button' name='devamsizlik_durumu' value='-1'>Azalt</button>
                </form>
